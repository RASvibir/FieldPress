...
export const EDITIONS: { id: PressieEdition; label: string; icon: string; desc: string }[] = [
  { id: 'tactical', label: 'Tactical Wire', icon: '⚡', desc: 'Standard: Field Broadsheet + Ink Stamps' },
  { id: 'vintage', label: '1920s Gazette', icon: '📰', desc: 'Old-Timey Broadsheet with Drop Caps' },
  { id: 'comic', label: 'Graphic Novel', icon: '💥', desc: 'Modern Comic Tabloid with Pop Badges' },
  { id: 'pixel', label: '8-Bit CRT', icon: '👾', desc: 'Retro Green Phosphor Teletype' },
  { id: 'sleek', label: 'Sleek Magazine', icon: '✨', desc: 'Minimalist Swiss Glass Editorial' },
];

export const PressieEditionSwitcher: React.FC<{
  currentEdition: PressieEdition;
  onSelectEdition: (edition: PressieEdition) => void;
}> = ({ currentEdition, onSelectEdition }) => {
  return (
    <div className="flex flex-wrap items-center gap-1.5 p-1.5 rounded-lg bg-black/60 border border-zinc-800 font-mono text-xs mb-4">
      <span className="text-[10px] text-zinc-500 uppercase font-bold px-2">Edition:</span>
      {EDITIONS.map(e => (
        <button
          key={e.id}
          type="button"
...
export const PressieArticleRenderer: React.FC<PressieArticleProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Interactive Edition Switcher Toolbar */}
      <PressieEditionSwitcher currentEdition={edition} onSelectEdition={setEdition} />
      ...
      {edition === 'tactical' && (
        <article className="p-6 rounded-lg border border-zinc-800 bg-zinc-950 font-mono relative overflow-hidden shadow-2xl">
          ...
          {/* Headline */}
          <h1 className="font-serif text-2xl sm:text-3xl font-bold text-white tracking-tight leading-snug mb-3">
            {story.title}
          </h1>

          {/* Prominent "FIELDED BY" Byline Banner */}
          ...
          {/* Article Body with AP-Style Dateline */}
          ...
          {/* Angled Physical Rubber Stamp Effect */}
      ...
      {/* ───────────────────────────────────────────────────────────── */}
      {edition === 'vintage' && (
        <article className="p-8 border-4 border-double border-amber-900/60 bg-[#14120e] text-[#dcd1ba] font-serif shadow-2xl relative">
          {/* Masthead */}
          <div className="text-center border-b-2 border-[#5c4a30] pb-3 mb-4">
            ...
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-[#f4ecd8] uppercase">
              {story.title}
            </h1>
          ...
          {/* Body with Drop Cap */}
          <div className="text-sm leading-relaxed space-y-4 columns-1 sm:columns-2 gap-6 text-justify">
            {leadText && (
              <p>
                <span className="float-left text-4xl leading-none font-black pr-2 pt-1 text-amber-500 font-serif">
                  {leadText.charAt(0)}
                </span>
          ...
          <div className="mt-4 pt-2 border-t border-[#5c4a30] text-[10px] font-mono text-center text-amber-700/80 uppercase">
            Printed & Preserved in Immutable Record Ledger • Public Domain Proof
          </div>
        </article>
      )}
      ...
      {/* ───────────────────────────────────────────────────────────── */}
      {edition === 'comic' && (
        <article className="p-6 border-4 border-black bg-zinc-900 text-white font-sans relative shadow-[8px_8px_0px_rgba(0,0,0,1)]">
          {/* Pop Banner */}
          ...
          <h1 className="text-2xl sm:text-3xl font-black uppercase tracking-tight text-white mb-3 leading-none drop-shadow-[2px_2px_0px_#000]">
            {story.title}
          </h1>
          ...
          {/* Comic Action Reaction Stamp */}
          <div className="mt-4 flex items-center space-x-2">
            <span className="bg-red-500 text-white font-black text-xs px-3 py-1 border-2 border-black uppercase transform rotate-2">
              🔥 {inkCounts['heat'] || 12} HEAT
            </span>
            <span className="bg-emerald-400 text-black font-black text-xs px-3 py-1 border-2 border-black uppercase transform -rotate-1">
              ⚡ {inkCounts['signal'] || 42} SIGNAL
            </span>
          </div>
        </article>
      )}
      ...
      {/* ───────────────────────────────────────────────────────────── */}
      {edition === 'pixel' && (
        <article className="p-6 border-2 border-emerald-500/80 bg-black text-emerald-400 font-mono text-xs relative shadow-[0_0_20px_rgba(16,185,129,0.25)]">
          {/* ASCII Top Bar */}
          <div className="text-[10px] text-emerald-500 mb-2 select-none">
            +=============================================================+<br />
          ...
          <h1 className="text-xl sm:text-2xl font-bold uppercase tracking-widest text-emerald-300 my-2">
            &gt; {story.title}
          </h1>

          <div className="p-2 border border-emerald-800 bg-emerald-950/30 my-3 text-[11px]">
            [OPERATOR_CREDENTIAL]: @{author} // STATUS: ACTIVE_ON_SCENE
          </div>

          <div className="space-y-3 leading-relaxed text-emerald-200/90 my-4">
            <p>&gt;&gt; {leadText}</p>
            {remainingText.map(t => (
              <p key={t.id}>&gt;&gt; {t.content}</p>
            ))}
          ...
          {photoItems.length > 0 && (
            <div className="border border-emerald-700 my-3 p-1 bg-black">
              ...
              <div className="text-[9px] text-emerald-500 text-center mt-1">
                --- [IMG_RASTER_ENCODED: PROOF_ATTACHED] ---
              </div>
            </div>
          )}

          <div className="pt-2 border-t border-emerald-800 text-[10px] flex justify-between items-center text-emerald-500">
            <span>SIG_PACKETS: {inkCounts['signal'] || 42}</span>
            <span>FORKS_ACK: OK</span>
          </div>
        </article>
      )}
      ...
      {/* ───────────────────────────────────────────────────────────── */}
      {edition === 'sleek' && (
        <article className="p-8 rounded-2xl border border-white/10 bg-zinc-900/70 backdrop-blur-xl font-sans text-zinc-100 shadow-2xl">
          {/* Category & Date */}
          ...
          {/* Headline */}
          <h1 className="text-3xl sm:text-4xl font-light tracking-tight text-white mb-4 leading-tight">
            {story.title}
          </h1>
